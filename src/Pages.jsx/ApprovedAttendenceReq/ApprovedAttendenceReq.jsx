...
import './ApprovedAttendenceReq.css'
import { viewApprovedReq } from '../../services/allApi'
import { MDBTable, MDBTableHead, MDBTableBody } from 'mdb-react-ui-kit';
import { useNavigate } from 'react-router-dom';

function ApprovedAttendenceReq() {
    ...
    const fetchAcceptedReq = async () => {
        ...
        const response = await viewApprovedReq(headers)
                    ...
                    <MDBTable responsive hover>
                        ...
                        <MDBTableBody>
                        {
                            data && data.length>0?data.map((attendenceData,index)=>(
                                <tr key={index}  onClick={() => navigate(`/attendence/approved/view/${attendenceData._id}`)} style={{ cursor: 'pointer' }}>
...
export default ApprovedAttendenceReq